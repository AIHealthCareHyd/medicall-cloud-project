...
const handler: Handler = async (event: HandlerEvent) => {
    ...
    try {
        ...
        const { data: updatedData, error: updateError } = await supabase.from('appointments').update({ appointment_date: newDate, appointment_time: newTime }).match({ patient_name: patientName, doctor_id: doctorData.id, appointment_date: oldDate, status: 'confirmed' }).select();
        ...
        return { statusCode: 200, headers, body: JSON.stringify({ success: true, message: `The appointment has been successfully rescheduled to ${newDate} at ${newTime}.` }) };
    } catch (error: any) {
